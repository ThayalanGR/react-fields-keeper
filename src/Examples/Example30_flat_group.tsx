                            ...
                            suffixNodeRenderer={({ fieldItem, bucketId }) => {
                                ...
                                const onOptionClick = (id: string, parentId?: string) => {
                                    setBuckets(prevBuckets => {
                                        const currentBucket = prevBuckets.find(bucket => bucket.id === bucketId);
                                        if (!currentBucket) return prevBuckets;

                                        let updatedItems: IFieldsKeeperItem[] = [...currentBucket.items];

                                        let insertIndex = updatedItems.findIndex(
                                            item => item.group === 'dateHierarchy' || item.id === 'date'
                                        );
                                        ...
                                        if (id === 'date') {
                                            ...
                                            const dateHierarchyItems = allItems
                                                .filter(item => item.group === 'dateHierarchy')
                                                .sort((a, b) => (a.groupOrder || 0) - (b.groupOrder || 0));
                                            updatedItems.splice(insertIndex, 0, ...dateHierarchyItems);
                                        }

                                        const updatedBucket = { ...currentBucket, items: updatedItems };
                                ...
                                const relevantContextMenuOptions =
                                    fieldItem.group === 'dateHierarchy' || fieldItem.id === 'date'
                                        ? contextMenuOptions
                                        : [];

                                return (
                                    <SuffixNode
                                        contextMenuOptions={relevantContextMenuOptions}
